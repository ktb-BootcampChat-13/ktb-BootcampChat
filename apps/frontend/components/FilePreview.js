...
import { Button, IconButton, Callout, Spinner } from '@vapor-ui/core';
...
const FilePreview = ({ 
  ...
  const renderUploadStatus = useCallback(() => {
    if (uploadError) {
      return (
        <Callout.Root colorPalette="danger" className="mt-2 flex items-center gap-2">
          <Callout.Icon>
    ...
    if (uploading) {
      return (
        <div className="mt-2 flex items-center gap-2 text-sm text-gray-600">
          <Spinner size="md" colorPalette="primary" aria-label="파일 업로드 중" />

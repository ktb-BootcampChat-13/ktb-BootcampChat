...
export const useFileHandling = (currentUser, roomId, handleSessionError) => {
  ...
  const handleFileUpload = useCallback(async (file, content = '') => {
    if (!socketClient.canSend() || !currentUser) {
    ...
    try {
      ...
      await socketClient.sendChatMessage({
        room: roomId,
        type: 'file',
        content: content,
        fileData: {
  ...
  }, [currentUser, roomId, handleSessionError]);
